...
export const GET = withAuth(async function(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await supabase.from('projects').select('*').eq('project_id', (await params).id).single();
...
export const PATCH = withAuth(async function(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const supabase = await getServerSupabase();
    const { data, error } = await supabase.from('projects').update(payload).eq('project_id', (await params).id).select().single();
...
export const DELETE = withAuth(async function(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const supabase = await getServerSupabase();
    const { error } = await supabase.from('projects').delete().eq('project_id', (await params).id);
    if (error) return NextResponse.json({ message: error.message }, { status: 400 });
